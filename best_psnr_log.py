...
log_contents = "../SR3/SR3_wdTest/experiments/s8_wd/logs/val.log"
...
for i in range(100):
    if max_psnr_entry[i][2] < 40500 and max_psnr_entry[i][0] == "val1":
...
for i in range(len(max_psnr_entry)):
    if max_psnr_entry[i][2] < 40500 and max_psnr_entry[i][0] == "val2":
        print(f"Validation: {max_psnr_entry[i][0]}, Iteration: {max_psnr_entry[i][2]}, PSNR: {max_psnr_entry[i][3]}")
        break
